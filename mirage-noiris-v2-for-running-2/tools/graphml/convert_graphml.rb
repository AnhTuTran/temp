...
graph[:RequestModels] = [
  {
    Id: "gamma",
    Model: "gamma",
    ParameterKeys: ["K", "Theta"],
    Parameters: [
      defaults[:RequestModels][:Gamma][:K],
      defaults[:RequestModels][:Gamma][:Theta]
    ]
  },
  {
    Id: "zipf",
    Model: "zipf",
    ParameterKeys: ["Skewness"],
    Parameters: [
      defaults[:RequestModels][:Zipf][:Skewness]
    ]
  }
]
...
doc.elements.each('graphml/graph/node') do |node_element|
  ...
  if label == 'origin'
    ...
    graph[:OriginServer] = {
      Id:          id,
      LibrarySize: defaults[:OriginServer][:LibrarySize]
    }
  else
    # それ以外のノードはキャッシュサーバとして扱う
    graph[:CacheServers] << {
      Id:             id,
      CacheAlgorithm: defaults[:CacheServers][:CacheAlgorithm],
      ParameterKeys:  cache_params[defaults[:CacheServers][:CacheAlgorithm].to_sym].keys.map(&:to_s),
      Parameters:     cache_params[defaults[:CacheServers][:CacheAlgorithm].to_sym].values
    }
  end
end
...
doc.elements.each('graphml/graph/edge') do |edge_element|
  ...
  graph[:Links] << {
    EdgeNodeIds:   [source, target],
    Cost:          defaults[:Links][:Cost],
    Bidirectional: defaults[:Links][:Bidirectional]
  }
end
...
graph[:CacheServers].each do |node|
  graph[:Clients] << {
    UpstreamId:     node[:Id],
    RequestModelId: defaults[:Clients][:RequestModelId],
    TrafficWeight:  defaults[:Clients][:TrafficWeight]
  }
end
